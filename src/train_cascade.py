...
batches = data_helpers.batch_iter(
    list(zip(x_train, 
        # author_train, topic_train, 
        y_train)), FLAGS.batch_size, FLAGS.num_epochs)
dev_batches = data_helpers.batch_iter(
    list(zip(x_dev, 
        # author_dev, topic_dev, 
        y_dev)), FLAGS.batch_size, FLAGS.num_epochs)
...
for batch in batches:
    ...
    t_loss, t_acc = train_step(x_batch, 
        # author_batch, topic_batch, 
        y_batch)
    ...
    if current_step % FLAGS.evaluate_every == 0:
        ...
        train_acc = []
        # Divide into batches
        dev_batches = data_helpers.batch_iter_dev(list(zip(x_dev, 
            # author_dev, topic_dev, 
            y_dev)), FLAGS.batch_size)
        dev_loss = []
        ll = len(dev_batches)
        ...
        for dev_batch in dev_batches:
            ...
            a, b = dev_step(x_dev_batch, 
                # author_dev_batch, topic_dev_batch, 
                y_dev_batch)
            dev_loss.append(a)
            conf_mat += b
        ...
        print(conf_mat)
        test_batches = data_helpers.batch_iter_dev(list(zip(x_test, 
            # author_test, topic_test, 
            y_test)), FLAGS.batch_size)
        test_loss = []
        conf_mat = np.zeros((2,2))
        for test_batch in test_batches:
            ...
            a, b = dev_step(x_test_batch, 
                # author_test_batch, topic_test_batch, 
                y_test_batch)
            test_loss.append(a)
            conf_mat += b
